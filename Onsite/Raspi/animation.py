            ...
            if not packet.text_being_displayed:
                ...
                for i in range(1, strands):
                    ...
                    if 3 * (packet.current_position - i) >= 0:
                        led_strand.color_state[3 * (packet.current_position - i)] = chr(int(float(ord(packet.red))/i))
                        led_strand.color_state[3 * (packet.current_position - i) + 1] = chr(int(float(ord(packet.green))/i))
                        led_strand.color_state[3 * (packet.current_position - i) + 2] = chr(int(float(ord(packet.blue))/i))
                    ...
                    led_strand.color_state[3*packet.current_position] = chr(int(float(ord(packet.red))/i))
                    led_strand.color_state[3*packet.current_position + 1] = chr(int(float(ord(packet.green))/i))
                    led_strand.color_state[3*packet.current_position + 2] = chr(int(float(ord(packet.blue))/i))
